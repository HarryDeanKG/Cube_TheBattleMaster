...
void ASmallMunition::Tick(float DeltaTime)
{
	...
	//FVector Position = FMath::VInterpConstantTo(Direction - GetActorLocation(), Direction - StartPosition, DeltaTime, Speed);
		if (GetLocalRole() == ROLE_Authority) {
			FVector Position = GetActorLocation() - FVector((StartPosition.X - Direction.X)*DeltaTime*Speed, (StartPosition.Y - Direction.Y) * DeltaTime*Speed, 0.f);
